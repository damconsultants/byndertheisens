...
class Psku extends \Magento\Backend\App\Action
{
     ...
     * @param \DamConsultants\BynderTheisens\Model\BynderConfigSyncDataFactory $byndersycData
    ...
    public function __construct(
        ...
        \DamConsultants\BynderTheisens\Model\BynderConfigSyncDataFactory $byndersycData,
    ...
    public function execute()
    {
		...
		try {
			...
			$result_data = $result->setData(['status' => 0, 'message' => $e->getMessage()]);
            return $result_data;
		}
    }
                ...
                if ($select_attribute == $data_value['type']) {
                    ...
                    if ($data_value['type'] == "image") {
                        ...
                        if ($select_attribute == 'video') {
                            $video_link = "";
                            if(isset($data_value["videoPreviewURLs"][0])){
                                $video_link = $data_value["videoPreviewURLs"][0] . '@@' . $image_data["thul"];
                            }else{
